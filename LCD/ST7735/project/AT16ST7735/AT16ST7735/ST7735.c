 ...
 */ 
 ...
 #include "ST7735.h"
 //=============================================Переменные========================================
 const char NewBFontLAT[] PROGMEM = {
...
PORTLightReg_LCD|=(1<<BL);
 ...
 void WriteChar8x11(unsigned char x0, unsigned char y0, unsigned char s, unsigned int color)
 {
	 ...
	 for(x=0;x<11;x++)
	 {
		 ...
		 c=0;
		 //unsigned char byte=pgm_read_byte(&(Font_8x11[s-32-c][x])); 
		 //if((byte&(1<<y))!=0)
		 OutColor(color);
		 //else
		 OutColor(BLACK);
 ...
 void WriteString8x11(unsigned char x0,unsigned char y0,unsigned char *s,unsigned int color)
 {
	 ...
	 for(i = 0;s[i]!='\0';i++)
	 {
		 ...
		 for(x=0;x<11;x++)
		 {
			 ...
			 c=0;
			//unsigned char byte=pgm_read_byte(&()); 
			 //if((pgm_read_byte(Font_8x11[s[i]-32-c][x])&(1<<y))!=0)
			 OutColor(color);
			 //else
			 OutColor(BLACK);
		 }
 ...
 void lcd_st7735_putchar(char x, char y, char chr,unsigned int  charColor, unsigned int bkgColor)
 {
	 ...
	 for (k=2;k>0;k--)
	 {
		 ...
		 for (j=0;j<h;j++)
		 {
			 ...
			 chl = chl<<1;
			 write_byte(((unsigned char)((color & 0xFF00)>>8)),1);
			 write_byte(((unsigned char) (color & 0x00FF)),1);
		 }
	 }
	 ...
	 for (j=0;j<13;j++)
	 {
		 write_byte(((unsigned char)((bkgColor & 0xFF00)>>8)),1);
		 write_byte(((unsigned char) (bkgColor & 0x00FF)),1);
	 }
